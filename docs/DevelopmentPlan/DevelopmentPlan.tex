...
11-23-2025 & Mohammad Mahdi Mahboob & Update POC Plan\\
... & ... & ...\\
\bottomrule
\end{tabularx}
\end{table}
...
The main challenges we foresee for this project are creating the form builder; user event registrations with QR code
check-ins; and exporting form response data, or using it to generate visualizations and analytics. The form-builder
needs to be modular and track all data against a user to perform analytics in order to minimize redundancy and improve
user experience. The event registration and check-in component is also a necessary feature outlined by the client. The
interface integration is also challenging because it demands an entirely different set of features for the client, yet
it must interact with the same underlying system.
\newline
\newline
The following are some significant risks we expect to encounter during development:
\begin{enumerate}
  \item \textbf{Implementation of Modular Forms} \\
    To reduce the size of forms while allowing for analytics over a broader set of form questions, we need to implement
    a way for admins to create forms, record responses, and query responses from the set of questions they wish. The
    risks involved in this process involve not displaying forms correctly on the user-side, an inadequate form builder
    on the admin side, as well as inadequate or inefficient form queries for admins.
    \newline
    \newline
    To mitigate this risk, we can study existing form building software to gather ideas of how builders are implemented,
    as well as study previous forms created by the MES to gauge the required complexity and depth of the feature. We also
    need to review and implement database concepts to allow for an adequate querying interface and to store form
    questions as persistent documents.
  \item \textbf{Form Data Exporting and Use in Analytics} \\
    As mentioned above, form data is used by admins to run analytics, and one of the features requested by MES is the
    ability to export form data into file formats such as CSVs. When accounting for modular forms, some risks that are
    encountered include incorrect queries of data, incorrect file formats, and incompatible data for analytics.
    \newline
    \newline
    To mitigate these risks, we plan to develop the module responsible for querying and exporting the form data
    according to admin preferences. This ensures that the intended form data is being exported, and will serve as an
    adequate baseline to ensure that analytics are conducted on the correct data.
  \item \textbf{Event Registration} \\
    Another key feature required by the MES is event registration, including user sign-ups and event-day check-ins. Some
    risks involved with this include inconsistency or volatile storage of QR codes, inability to track event check-ins
    correctly, and inability to register multiple instances of a user for an event.
    \newline
    \newline
    To mitigate this risk, we intend to develop a consistent formula to convert event and user information into some
    unique string which is stored in our database and embedded in the QR code used for registration. When the code is
    scanned, the users can successfully be checked-in to the system and their entry tracked so that redundant scans are
    impermissible.
\end{enumerate}
