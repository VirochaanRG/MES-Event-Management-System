...
04-06-2026 & Virochaan & Implemented TA Feedback and Peer Review 86, Proper traceability in section 4.3 for NFR \\
04-06-2026 & Virochaan & Implemented TA Feedback, Document Formatting and reviewing document \\
04-06-2026 & Virochaan & Implemented Peer Review 84, Fixed grammer and spelling across the document\\
04-06-2026 & Virochaan & Implemented Peer Review 87, Revamped all test cases in section 4.1 to be less ambiguous as neccesary\\
04-06-2026 & Virochaan & Implemented Peer Review 88, In section 4.2 ensure that all the NFRs are properly covered for thoroughness\\
04-06-2026 & Virochaan & Implemented Peer Review 93, In section 4.2.4 ensure proper measurement criterion for the test\\
04-06-2026 & Virochaan & Implemented Peer Review 89, Include link to Usability Survey Questions\\
\bottomrule
\end{tabularx}
...
All users will interact regularly with the system through different interfaces so evaluating them through the lens of these design principles is good for ensuring effective design. It will also make sure these interactions are productive, making this extra relevant to the project.
...
\begin{itemize}
  ...
  \item \textbf{Luke Schermann}: Supervisor and Acceptance Tester. Supervises the overall implementation of the system, ensuring stakeholder requirements are met. Responsible for attending acceptance testing workshops with the developers for the admin web portal.
\end{itemize}
...
\subsubsection{Stakeholder Review}
  ...
  User acceptance testing will be conducted on a staging deployment of the system with a group of about 10 test users. Each user will be given a list of actions to perform on the attendee mobile application without help from developers, such as looking up and registering for an event or filling out a survey. The user will then provide feedback on the experience of completing these actions which will be analyzed for final improvements before the initial launch.
...
For the formal design review, the supervisor will be presented with a detailed walkthrough of the system architecture and rationale for key design decisions. This presentation will cover the entire system architecture with an emphasis on the admin perspective of system. The supervisor will provide feedback and propose changes to ensure the design aligns with the interest of major stakeholders.
...
Usability verification will follow the same framework used in the Usability Report Extra. This keeps both deliverables consistent.

The team will run task-based sessions with MES members and student attendees. Core tasks will include event registration, form completion, and admin analytics review. We will evaluate each task using clear metrics. These metrics include completion rate, time on task, observed errors, and user satisfaction score.

The team will record both quantitative and qualitative results. Quantitative results include completion percentage and timing data. Qualitative results include confusion points and wording/navigation issues. We will compare results against the success thresholds defined in the Usability Report. Any needed improvements will be tracked as GitHub Issues before final release.
...
This section outlines the system-wide tests to validate both functional and non-functional requirements defined in our Software Requirements Specification.
...
\textbf{Input/Condition:} Execute at least three sub-cases: (A) create a valid form containing multiple field types (for example text, checkbox, and rating), (B) submit a blank/whitespace-only form title, and (C) submit a title that exceeds the configured maximum length.\\
\textbf{Expected Output:} (A) Form is created with a unique identifier, appears in the admin dashboard, and the persisted schema matches the configured field types/constraints. (B) and (C) are rejected with validation errors, and no new form record is created.\\
...
\textbf{How the test will be performed:} Tester records form count before/after each sub-case, validates field schema persistence for the valid case, and confirms no count increase for invalid cases.\\[6pt]
...
\textbf{Input/Condition:} Select multiple forms with known seeded response sets and include one form with no responses (or archived status) as a negative case.\\
\textbf{Expected Output:} Combined totals and per-answer distributions shown on analytics match backend aggregate results for the selected forms; zero-response forms do not inflate totals or produce incorrect chart values.\\
...
\textbf{How the test will be performed:} Tester computes reference aggregates through SQL/API and compares dashboard metrics (totals and distributions) metric-by-metric with pass/fail logging.\\[6pt]

\noindent\textbf{Test-FR-AP-3: Manage Events via Dashboard}\\
\textbf{Type:} Manual\\
\textbf{Initial State:} Several events with participant data exist.\\
\textbf{Input/Condition:} Use an event dataset containing mixed payment/status states. Apply single filters (for example pending, paid, cancelled), combined filters, and a filter combination expected to return no rows.\\
\textbf{Expected Output:} Each filter view returns only matching events with correct counts; non-matching events are excluded; no-result scenarios show a clear empty state and do not retain stale rows from previous filter states.\\
...
\textbf{How the test will be performed:} Tester captures displayed event IDs/counts for each filter and cross-checks them with baseline seeded data and backend query output.\\[6pt]
...
\textbf{Input/Condition:} Select an event with known non-zero analytics and export report data in supported formats (for example CSV/XLSX). Include a negative case for an event with no feedback data.\\
\textbf{Expected Output:} On-screen analytics values match backend totals. Exported files are generated successfully with expected headers and data consistency. For no-feedback cases, output is valid and explicitly represents zero/no data without corruption.\\
...
\textbf{How the test will be performed:} Tester verifies on-screen metrics first, then validates exported file structure and key totals against dashboard/backend values.\\[12pt]
...
\textbf{Input/Condition:} Execute three flows: (A) valid attendee registration to an open event, (B) duplicate registration attempt for the same attendee/event pair, and (C) invalid payload missing a required field.\\
\textbf{Expected Output:} (A) succeeds and creates one registration with associated confirmation and ticket artifact; (B) and (C) are rejected with appropriate validation/duplicate responses and do not create additional records.\\
...
\textbf{How the test will be performed:} Automated test asserts status codes, record-count changes, and creation/non-creation of registration artifacts per sub-case.\\[6pt]

\noindent\textbf{Test-FR-AA-2: Access Event Listings}\\
\textbf{Type:} Manual\\
\textbf{Initial State:} The system has multiple available and past events.\\
\textbf{Input/Condition:} Use a user account with a mixed event set (available/past, registered/not registered). Open \texttt{Available} and \texttt{Registered Events} tabs.\\
\textbf{Expected Output:} Each tab shows only events that satisfy its rules; events outside the rule set are excluded; displayed event metadata (title/date/location/status) matches backend records.\\
...
\textbf{How the test will be performed:} Tester records event IDs in each tab, compares against expected membership, and validates displayed metadata against backend values.\\[6pt]

\noindent\textbf{Test-FR-AA-3: Fill out Form}\\
\textbf{Type:} Automatic\\
\textbf{Initial State:} A survey is available and attendee is authenticated.\\
\textbf{Input/Condition:} Execute three survey cases: (A) valid completion with all required fields, (B) submission missing at least one required field, and (C) submission violating an input-length constraint.\\
\textbf{Expected Output:} (A) persists a completed response and returns success/confirmation; (B) and (C) return validation errors and do not persist a completed response.\\
...
\textbf{How the test will be performed:} Automated suite checks response status/message and verifies persistence outcomes for each case against expected behavior.\\[12pt]
...
\textbf{Input/Condition:} Admin performs at least two updates to attendee-visible event fields (for example venue, time, description) while attendee view is open. Propagation delay is measured without manual refresh.\\
\textbf{Expected Output:} Attendee view reflects updates automatically within the agreed synchronization threshold and values match admin/backend state with no stale data persisting after update completion.\\
...
\textbf{How the test will be performed:} Automation timestamps update and visibility events, then validates propagation-time and field-equality assertions across admin UI, attendee UI, and backend records.\\[6pt]
...
\textbf{Input/Condition:} Run a role matrix where admin and attendee users attempt the same set of admin-only UI actions and direct API/URL accesses.\\
\textbf{Expected Output:} Admin role is authorized for admin-only actions. Attendee role is consistently denied (unauthorized/forbidden), receives no restricted data payload, and denial attempts are captured by logging/audit mechanisms.\\
...
\textbf{How the test will be performed:} Tester executes a shared action matrix for both roles, records outcomes per action, and verifies both access-control results and audit logging behavior.\\[12pt]
...
\begin{enumerate}[label=\bfseries UH-\arabic*:, wide=0pt]
  ...
  \item \label{test-UH4} \textbf{Understandable Language and Clear Prompts}\\[2mm]
    {\bf Type:} Manual, Dynamic\\
    ...
    {\bf Output/Result:} 80\% of users complete survey and rate language as acceptable.\\
    {\bf Covers:} UR1, UR2, CR1\\
  ...
  \item \label{test-UH6} \textbf{Accessibility Conformance}\\[2mm]
    ...
    {\bf Covers:} AC1\\
    {\bf How test will be performed:} Automated web tools will be used to assess conformance to accessibility standards (WebAIM).
\end{enumerate}

\subsubsection{Robustness and Fault Tolerance}
\begin{enumerate}[label=\bfseries RF-\arabic*:, wide=0pt]
  \item \label{test-RF1} \textbf{Survey Auto-Save Reliability}\\[2mm]
    {\bf Type:} Automatic, Dynamic\\
    {\bf Initial State:} A multi-question survey is open with partially filled responses.\\
    {\bf Input/Condition:} Enter partial responses, wait at least one auto-save interval, then refresh or relaunch the client.\\
    {\bf Output/Result:} Previously entered responses are restored and no completed-submission state is incorrectly triggered.\\
    {\bf Covers:} FT1, FT2\\
    {\bf How test will be performed:} Use automated browser/device tests to compare field values before and after refresh and confirm state persistence.

  \item \label{test-RF2} \textbf{Template Update Resilience}\\[2mm]
    {\bf Type:} Automatic, Dynamic\\
    {\bf Initial State:} Admin and attendee traffic is active in staging.\\
    {\bf Input/Condition:} Add or update templates/modules while concurrent sessions are active.\\
    {\bf Output/Result:} Update succeeds, new template state is reflected correctly, and active sessions are not disrupted.\\
    {\bf Covers:} FT3, FT4\\
    {\bf How test will be performed:} Run concurrent update and usage scenarios, then verify both update correctness and uninterrupted user operations.
\end{enumerate}
...
\begin{enumerate}[label=\bfseries MS-\arabic*:, wide=0pt]
  ...
  \item \label{test-MS3} \textbf{Extensibility and Maintainability Review}\\[2mm]
    {\bf Type:} Manual, Static\\
    {\bf Initial State:} Current architecture and module boundaries are documented.\\
    {\bf Input/Condition:} Perform a structured review by adding a small analytics extension in a feature branch and evaluating integration effort and isolation of changes.\\
    {\bf Output/Result:} Extension can be integrated without large refactors, and architecture/docs remain maintainable across framework updates.\\
    {\bf Covers:} SE2, LG1, LG2\\
    {\bf How test will be performed:} Use review checklist and change-scope metrics (files touched, coupling points) to assess extensibility and maintainability.
\end{enumerate}
...
\begin{enumerate}[label=\bfseries CM-\arabic*:, wide=0pt]
  \item \label{test-CM1} \textbf{PIPEDA Compliance Validation}\\[2mm]
    {\bf Type:} Manual, Static\\
    {\bf Initial State:} Data handling flows and privacy controls are documented.\\
    {\bf Input/Condition:} Perform checklist-based review of collection, consent, storage, access, and deletion controls against PIPEDA-aligned criteria.\\
    {\bf Output/Result:} Compliance checklist passes or produces actionable remediation items with owners.\\
    {\bf Covers:} CLR1, CSCR1\\
    {\bf How test will be performed:} Security/compliance reviewer traces implemented controls to policy checklist and records evidence links.
\end{enumerate}
...
These tests have traceability with our SRS document which can be found at the following link: \href{https://github.com/VirochaanRG/MES-Event-Management-System/blob/main/docs/SRS/SRS.pdf}{SRS Document}
\begin{table}[H]
\centering
\begin{tabular}{|l|l|}
\hline
...
\hline
Test-NFR-LF-1 & AR1 \\
Test-NFR-LF-2 & AR2, SR1 \\
Test-NFR-LF-3 & AR3, SR2, FT5 \\
Test-NFR-UH-1 & ER1, LR1 \\
Test-NFR-UH-2 & ER2 \\
Test-NFR-UH-3 & SR2, ER3, PI1, PI2, LR2 \\
Test-NFR-UH-4 & UR1, UR2, CR1 \\
Test-NFR-UH-5 & UR2 \\
Test-NFR-UH-6 & AC1 \\
Test-NFR-RF-1 & FT1, FT2 \\
Test-NFR-RF-2 & FT3, FT4 \\
Test-NFR-PF-1 & SL1, MSAR1, MSAR2 \\
Test-NFR-PF-2 & SL2, PA1 \\
Test-NFR-PF-4 & SE1 \\
Test-NFR-OE-1 & RR1 \\
Test-NFR-MS-1 & MSMR1 \\
Test-NFR-MS-2 & MSSR1 \\
Test-NFR-MS-3 & SE2, LG1, LG2 \\
Test-NFR-ST-1 & SAR1 \\
Test-NFR-ST-2 & SIR2 \\
Test-NFR-ST-3 & SPR1 \\
Test-NFR-ST-4 & SPR2, SPR3, SC1 \\
Test-NFR-ST-5 & SAuR1 \\
Test-FR-SI-2 & SAR2, UR3 \\
Test-FR-AP-1 & SIR1 \\
Test-FR-AA-1 & SIR1 \\
Test-FR-AA-3 & SIR1 \\
Test-NFR-CM-1 & CLR1, CSCR1 \\
\hline
\end{tabular}
...
\begin{itemize}
  ...
  \item \textbf{M10 - Report Generation Module}: Report quality depends on aggregated cross-module data and formatting/export pipelines. It is validated through functional/system analytics export tests that assess completeness and ensure the outputted files are of the correct format.
...
\item \textbf{What pain points did you experience during this deliverable, and how did you resolve them?} \
A notable challenge was connecting all the tests for the nonfunctional and functional requirements without causing overlap or redundancy. Many of the NFRs would have already been covered in the tests for functional requirements, so there was a lot of retracing steps in order to dictate which NFRs remained to be tested. Each test needed to have a distinct purpose while still providing competent coverage. By extension, also ensuring that the tests were feasible, but still good enough to satisfy the criteria was slightly difficult as we want to ensure the testing remains within the scope of the project.
\end{enumerate}
