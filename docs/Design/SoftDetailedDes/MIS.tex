...
\externaldocument{../../Design/SoftArchitecture/MG}[MG]
...
Map of type K to V & map(K, V) & A map containing key value pairs with keys of type K and values of type V\\
Date and time & DateTime & Stores a specific date in time using the Gregorian calendar and 24 hour clock\\
User session & Session & Stores data on a user's session and time of last validation\\
Access Role & Role & Defines a role for resource and feature access control\\
Form Module & FormModule & Contains data for a form module such as a list of questions\\
Question & Question & Contains data for a form question such as the question itself and possible answers\\
\bottomrule
\end{tabular}
\end{center}
...
\section{MIS of M\ref{m2}: User Authorization Module}
...
\texttt{roles : map(string, set(T))}: Mapping of user IDs to the set of roles the user is assigned.
...
\hline
\texttt{removeModule} & \texttt{moduleId : $\mathbb{N}$} & None & \texttt{InvalidModule} \\
\hline
...
\noindent \texttt{removeModule(moduleId : $\mathbb{N}$)}: \\
Creates a new module given a title and a list of questions.
\begin{itemize}
...
\noindent \texttt{linkQuestion(question : Question, linkedModuleIds : list(string))}: \\
...
\noindent \texttt{getFillableForms(userId : string)}:
\begin{itemize}
\item transition: None 
...
\noindent \texttt{respondToForm(userId : string, formId: string, \newline formResponse: map(Question, string))}:
\begin{itemize}
...
\section{MIS of M\ref{m7}: Registration Module}
